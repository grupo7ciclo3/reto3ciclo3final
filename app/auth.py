    ...
    try:
        ...
        if request.method == 'GET': 
            ...
            if attempt is not None:
                db.execute(
                    'SELECT id FROM user WHERE username=?', (utils.U_CONFIRMED, attempt['id'])
                )
                db.execute(
...
def confirm():
    try:
        ...
        if request.method == 'POST': 
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT id FROM user WHERE username=?', (authid, utils.F_ACTIVE)
            ).fetchone()
            
            if attempt is not None:
                db.execute(
                    'SELECT id FROM user WHERE username=?', (utils.F_INACTIVE, attempt['id'])
                )
                salt = hex(random.getrandbits(128))[2:]
                hashP = generate_password_hash(password + salt)   
                db.execute(
                    'SELECT id FROM user WHERE username=?', (hashP, salt, attempt['userid'])
                )
                db.commit()
                return redirect(url_for('auth.login'))
            else:
...
def change():
    try:
        ...
        if request.method == 'GET': 
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT id FROM user WHERE username=?', (number, utils.F_ACTIVE)
            ).fetchone()
